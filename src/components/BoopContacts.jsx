...
const BoopContacts = ({ users, setUsers, posts, setPosts }) => {
	...
	const removeUserFromList = (id) => {
		if (
			window.confirm(
				"✋ Removing a contact will also remove all there posts from your feed... \r\n 🤔 Do you want to remove?"
			)
		) {
		...
		<div id="contacts-menu" className="boop-contacts">
			...
			<div className="contacts-title-row">
				...
				<div className="contacts-options">
					<div onClick={() => addNewUser()} className="new-room">
						...
						return (
							<div key={idx} className="user-contact">
								{user.id === 1 ? (
									...
									<img
										src={`https://randomuser.me/api/portraits/thumb/women/${user.id}.jpg`}
										alt="contact"
									/>
								)}
								{user.name}
